...
-- Safe to re-run: uses "if not exists" / "if exists" guards where practical,
-- but the seed INSERT at the bottom will duplicate items if run twice —
-- only run this file once on a fresh project.
